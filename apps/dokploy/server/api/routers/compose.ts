...
export const composeRouter = createTRPCRouter({
	...
	deployTemplate: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const serviceDisplayName = input.name || input.id;
			const projectName = slugify(`${project.name} ${serviceDisplayName}`);
			...
			if (input.autoDeploy) {
				const jobData: DeploymentJob = {
					composeId: compose.composeId,
					titleLog: `Initial deployment of ${compose.name}`,
					type: "deploy",
					applicationType: "compose",
					descriptionLog: `Deploying ${compose.name} game server container`,
					server: !!compose.serverId,
					serverId: compose.serverId ?? undefined,
				};
				if (IS_CLOUD && compose.serverId) {
