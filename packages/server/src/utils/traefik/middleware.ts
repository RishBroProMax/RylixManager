...
import { getDefaultMiddlewares } from "../../setup/traefik-setup";
...
export const ensureSecurityMiddlewares = async (serverId?: string) => {
	let config: FileConfig;
	try {
		config = serverId
	...
	const defaultMiddlewaresYaml = getDefaultMiddlewares();
	const parsedDefault = parse(defaultMiddlewaresYaml) as FileConfig;
	const defaultList = parsedDefault?.http?.middlewares || {};

	let modified = false;
	for (const [name, definition] of Object.entries(defaultList)) {
		if (!config.http.middlewares[name]) {
			config.http.middlewares[name] = definition;
			modified = true;
		}
	}
